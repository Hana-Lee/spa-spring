...
app.chat = (function() {
	...
	var configMap = {
		main_html : String() + '<div class="app-chat">'
				+ '<div class="app-chat-head">'
				+ '<div class="app-chat-head-toggle">+</div>'
				+ '<div class="app-chat-head-title">' + 'Chat' + '</div>'
				+ '</div>' + '<div class="app-chat-closer">x</div>'
				+ '<div class="app-chat-sizer">'
				+ '<div class="app-chat-list">'
				+ '<div class="app-chat-list-box"></div>' + '</div>'
				+ '<div class="app-chat-msg">'
				+ '<div class="app-chat-msg-log"></div>'
				+ '<div class="app-chat-msg-in">'
				+ '<form class="app-chat-msg-form">' + '<input type="text"/>'
				+ '<input type="submit" style="display:none"/>'
				+ '<div class="app-chat-msg-send">' + 'send' + '</div>'
	...
	setJqueryMap, setPxSizes, scrollChat, writeChat, writeAlert, clearChat, setSliderPosition, onTapToggle, onSubmitMsg, onTapList, onSetchatee, onUpdatechat, onListchange, onLogin, onLogout, configModule, initModule, removeSlider, handleResize;
	...
	setJqueryMap = function() {
		var $append_target = stateMap.$append_target, $slider = $append_target
				.find('.app-chat');
		...
		jqueryMap = {
			...
			//$list_box : document.getElementsByClassName('app-chat-list-box'),
			$msg_log : $slider.find('.app-chat-msg-log'),
			$msg_in : $slider.find('.app-chat-msg-in'),
			$input : $slider.find('.app-chat-msg-in input[type=text]'),
			$send : $slider.find('.app-chat-msg-send'),
			$form : $slider.find('.app-chat-msg-form'),
			$window : $(window)
		};
	};
	...
	setPxSizes = function() {
		...
		window_height_em = Math
				.floor((jqueryMap.$window.height() / px_per_em) + 0.5);

		opened_height_em = window_height_em > configMap.window_height_min_em ? configMap.slider_opened_em
	...
	// Example : app.chat.setSliderPosition( 'closed' );
	...
	writeChat = function(person_name, text, is_user) {
		var msg_class = is_user ? 'app-chat-msg-log-me'
				: 'app-chat-msg-log-msg';

		jqueryMap.$msg_log.append('<div class="' + msg_class + '">'
				+ app.util_b.encodeHtml(person_name) + ': '
				+ app.util_b.encodeHtml(text) + '</div>');

		scrollChat();
	};

	writeAlert = function(alert_text) {
		jqueryMap.$msg_log.append('<div class="app-chat-msg-log-alert">'
				+ app.util_b.encodeHtml(alert_text) + '</div>');
		scrollChat();
	};
	...
	onSetchatee = function(event, arg_map) {
		...
		jqueryMap.$list_box.find('.app-chat-list-name').removeClass(
				'app-x-select').end().find(
				'[data-id=' + arg_map.new_chatee.id + ']').addClass(
				'app-x-select');
	...
	onListchange = function(event) {
		var list_html = String(), people_db = configMap.people_model.get_db(), chatee = configMap.chat_model
				.get_chatee();
		...
		people_db().each(
				function(person, idx) {
					...
					list_html += '<div class="app-chat-list-name'
							+ select_class + '" data-id="' + person.id + '">'
							+ app.util_b.encodeHtml(person.name) + '</div>';
				});

		if (!list_html) {
			list_html = String() + '<div class="app-chat-list-note">'
	...
	onUpdatechat = function(event, msg_map) {
		var is_user, sender_id = msg_map.sender_id, msg_text = msg_map.msg_text, chatee = configMap.chat_model
				.get_chatee()
	...
	// Example : app.chat.configModule({ slider_open_em : 18 });
	...
	// Example : app.chat.initModule( $('#div_id') );
	// Purpose :
	...
	initModule = function($append_target) {
		...
		app.util_b.gevent.subscribe($list_box, 'app-listchange', onListchange);
		app.util_b.gevent.subscribe($list_box, 'app-setchatee', onSetchatee);
		app.util_b.gevent.subscribe($list_box, 'app-updatechat', onUpdatechat);
		app.util_b.gevent.subscribe($list_box, 'app-login', onLogin);
		app.util_b.gevent.subscribe($list_box, 'app-logout', onLogout);
