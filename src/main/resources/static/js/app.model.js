...
app.model = (function() {
	...
	// ---------------------
	// The people object is available at app.model.people.
	...
	// publishes a 'app-login' global custom event.
	// * logout()- revert the current user object to anonymous.
	// This method publishes a 'app-logout' global custom event.
	//
	// jQuery global custom events published by the object include:
	// * app-login - This is published when a user login process
	// completes. The updated user object is provided as data.
	// * app-logout - This is published when a logout completes.
	...
	completeLogin = function(user_list) {
		...
		chat.join();
		app.util_b.gevent.publish('app-login', [ stateMap.user ]);
	};

	makePerson = function(person_map) {
		var person, cid = person_map.cid, css_map = person_map.css_map, id = person_map.id, name = person_map.name;
	...
	people = (function() {
		...
		login = function(name) {
			var sio = isFakeData ? app.fake.mockSio : app.data.getSio();
		...
		logout = function() {
			...
			app.util_b.gevent.publish('app-logout', [ user ]);
		};
	...
	// The chat object API
	// -------------------
	// The chat object is available at app.model.chat.
	...
	// for 'app-listchange' and 'app-updatechat' global
	...
	// It publishes a 'app-setchatee' global custom event.
	// * send_msg( <msg_text> ) - send a message to the chatee.
	// It publishes a 'app-updatechat' global custom event.
	...
	// 'app-listchange' event which publishes the updated
	...
	//
	// jQuery global custom events published by the object include:
	// * app-setchatee - This is published when a new chatee is
	// set. A map of the form:
	// { old_chatee : <old_chatee_person_object>,
	// new_chatee : <new_chatee_person_object>
	// }
	// is provided as data.
	// * app-listchange - This is published when the list of
	...
	// * app-updatechat - This is published when a new message
	...
	chat = (function() {
		...
		_update_list = function(arg_list) {
			var i, person_map, make_person_map, person, people_list = arg_list[0], is_chatee_online = false;
			...
			// which triggers the 'app-setchatee' global event
		...
		_publish_listchange = function(arg_list) {
			...
			app.util_b.gevent.publish('app-listchange', [ arg_list ]);
		};
		...
		_publish_updatechat = function(arg_list) {
			...
			app.util_b.gevent.publish('app-updatechat', [ msg_map ]);
		};
		...
		join_chat = function() {
			...
			sio = isFakeData ? app.fake.mockSio : app.data.getSio();
		...
		send_msg = function(msg_text) {
			var msg_map, sio = isFakeData ? app.fake.mockSio : app.data
					.getSio();
		...
		set_chatee = function(person_id) {
			...
			app.util_b.gevent.publish('app-setchatee', {
				old_chatee : chatee,
